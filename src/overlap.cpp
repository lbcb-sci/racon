...
Overlap::Overlap(std::uint32_t q_id, std::uint32_t q_begin, std::uint32_t q_end,
    std::uint32_t t_id, std::uint32_t t_begin, std::uint32_t t_end,
    std::uint32_t strand)
        : q_id(q_id), q_begin(q_begin), q_end(q_end), t_id(t_id), t_begin(t_begin),
        t_end(t_end), strand(strand), cigar(), break_points() {
}
...
void Overlap::find_break_points(
...
void Overlap::find_break_points(std::uint32_t w) {

    // find break points from cigar
    ...
    std::pair<std::uint32_t, std::uint32_t> first_match = {0, 0}, last_match = {0, 0};
...
void Overlap::align(const char* q, std::uint32_t q_length, const char* t,
    ...
    if (result.status == EDLIB_STATUS_OK) {
        char* ret = edlibAlignmentToCigar(result.alignment,
            result.alignmentLength, EDLIB_CIGAR_STANDARD);
        cigar = ret;
        free(ret);
    } else {
        throw std::invalid_argument("[racon::Overlap::find_breaking_points] error: "
            "edlib unable to align pair " + std::to_string(q_id) + " and " +
            std::to_string(t_id));
    }
